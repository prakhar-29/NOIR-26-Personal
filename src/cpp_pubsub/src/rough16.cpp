...
private:
    ...
    const float CONE_SEARCH_RADIUS = 1.0;      // Start cone search within 1m
    
    // Camera parameters
    const float CAMERA_WIDTH = 640.0;          // Camera resolution width
    const float CAMERA_HEIGHT = 640.0;         // Camera resolution height
    const float CAMERA_CENTER_X = CAMERA_WIDTH / 2.0;
    const float ALIGNMENT_TOLERANCE = 75.0;    // Pixels
    const float CONE_DETECTION_TIMEOUT = 1.0;  // Seconds
    
    // Cone approach parameters
    const double CONE_STOP_THRESHOLD = 0.5;    // Stop when 0.5m away
    const double CONE_LINEAR_VEL = 0.15;       // Linear velocity towards cone
    const double CONE_ROTATION_VEL = 0.15;     // Rotation velocity for searching
    const double CONE_ALIGN_KP = 0.003;        // Proportional gain for alignment
    const double MAX_ALIGN_ANGULAR_VEL = 0.25; // Maximum angular velocity
    const double smooth_factor = 0.6;              // overall smoothener for the whole mission
    ...
    {
        // Index 0 is placeholder, actual classes are 1-7
    ...
    void cone_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
    {
        ...
        // Format: [cone_index, x, y, color_id, ...] or [0, 0, 0, 0] for no detection
        if (msg->data.size() < 4) {
            return;
        }
    
        // Check for "no detection" message
        ...
        // Parse detections - format: [cone_index, x, y, color_id] repeating
        for (size_t i = 0; i + 3 < msg->data.size(); i += 4)
        {
            float cone_index = msg->data[i];      // Cone number (1, 2, 3...)
            float cx = msg->data[i + 1];          // X coordinate (pixels)
            float cy = msg->data[i + 2];          // Y coordinate (pixels)
            float class_id = msg->data[i + 3];    // Color class (1-7)
        
            // Filter by target cone class
            if (is_valid_cone_class(static_cast<int>(class_id)))
            {
                // Find cone closest to center of camera
