...
private:
    ...
    // Drop sequence state
    bool drop_sequence_active_{false};
    int drop_sequence_phase_{0};  // 0=shoulder down, 1=shoulder stop, 2=gripper open, 3=gripper stop
    rclcpp::Time drop_sequence_start_time_;
    ...
    void drop_object()
    {
        RCLCPP_INFO(get_logger(), "Starting drop sequence...");
        RCLCPP_INFO(get_logger(), "Phase 1: Shoulder moving down...");
        
        // Store start time and set flag
        drop_sequence_start_time_ = this->now();
        drop_sequence_active_ = true;
        drop_sequence_phase_ = 0;  // 0=shoulder down, 1=shoulder stop, 2=gripper open, 3=gripper stop
        
        RCLCPP_INFO(get_logger(), "Drop sequence started...");
    }

    /* -------- DROP SEQUENCE UPDATE (called from gps_callback) -------- */
    void update_drop_sequence()
    {
        if (!drop_sequence_active_) return;
        
        geometry_msgs::msg::Twist arm_cmd;
        arm_cmd.linear.x = 0.0;
        arm_cmd.linear.y = 0.0;
        arm_cmd.linear.z = 0.0;
        arm_cmd.angular.x = 0.0;
        arm_cmd.angular.y = 0.0;
        arm_cmd.angular.z = 0.0;
        
        auto elapsed = (this->now() - drop_sequence_start_time_).seconds();
        
        if (drop_sequence_phase_ == 0) {
            // Phase 0: Shoulder down for 4 seconds
            if (elapsed < 4.0) {
                ...
                drop_sequence_start_time_ = this->now();
                RCLCPP_INFO(get_logger(), "Shoulder down complete. Stopping shoulder...");
            }
        } 
        else if (drop_sequence_phase_ == 1) {
            // Phase 1: Shoulder stop for 2 seconds
            arm_cmd.angular.x = 0.0;  // Stop shoulder
            arm_cmd_pub_->publish(arm_cmd);
            
            if (elapsed < 2.0) {
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
                    "Shoulder stopped... %.1f seconds remaining", 2.0 - elapsed);
            } else {
                // Switch to gripper open phase
                drop_sequence_phase_ = 2;
                drop_sequence_start_time_ = this->now();
                RCLCPP_INFO(get_logger(), "Shoulder stopped. Opening gripper...");
            }
        }
        else if (drop_sequence_phase_ == 2) {
            // Phase 2: Gripper open for 4 seconds
            if (elapsed < 4.0) {
                arm_cmd.linear.y = -    -1.0;  // Open gripper
                arm_cmd_pub_->publish(arm_cmd);
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
                    "Gripper opening... %.1f seconds remaining", 4.0 - elapsed);
            } else {
                // Switch to gripper stop phase
                drop_sequence_phase_ = 3;
                drop_sequence_start_time_ = this->now();
                RCLCPP_INFO(get_logger(), "Gripper opened. Stopping gripper...");
            }
        }
        else if (drop_sequence_phase_ == 3) {
            // Phase 3: Gripper stop for 2 seconds
            arm_cmd.linear.y = 0.0;  // Stop gripper
            arm_cmd_pub_->publish(arm_cmd);
            
            if (elapsed < 2.0) {
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
                    "Gripper stopped... %.1f seconds remaining", 2.0 - elapsed);
            } else {
                // Complete - send final 0.0 signal, mark as done, and shutdown
                drop_sequence_active_ = false;
                RCLCPP_INFO(get_logger(), "Object dropped successfully! All operations stopped.");
    ...
    {
        // Update drop sequence if active
        update_drop_sequence();
        ...
        {
            if (!drop_sequence_active_) {
                // Start the drop sequence
                drop_object();
            }
            // The update_drop_sequence() at the start of this callback handles the rest
            return;
        }
