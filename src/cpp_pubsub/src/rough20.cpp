...
private:
    ...
    // Gripper drop state
    bool gripper_drop_active_{false};
    int gripper_drop_phase_{0};
    rclcpp::Time gripper_drop_start_time_;
    ...
    void drop_object()
    {
        ...
        gripper_drop_start_time_ = this->now();
        gripper_drop_active_ = true;
        gripper_drop_phase_ = 0;  // 0 = opening, 1 = stopping
        
        RCLCPP_INFO(get_logger(), "Gripper drop sequence started...");
    }

    /* -------- GRIPPER DROP UPDATE (called from gps_callback) -------- */
    void update_gripper_drop()
    {
        if (!gripper_drop_active_) return;
        ...
        auto elapsed = (this->now() - gripper_drop_start_time_).seconds();
        
        if (gripper_drop_phase_ == 0) {
            // Opening phase (4 seconds)
            if (elapsed < 4.0) {
                gripper_cmd.linear.y = 1.0;  // Open
                arm_cmd_pub_->publish(gripper_cmd);
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
                    "Gripper opening... %.1f seconds remaining", 4.0 - elapsed);
            } else {
                // Switch to stopping phase
                gripper_drop_phase_ = 1;
                gripper_drop_start_time_ = this->now();
                RCLCPP_INFO(get_logger(), "Gripper opened. Stopping gripper motion...");
            }
        } else if (gripper_drop_phase_ == 1) {
            // Stopping phase (2 seconds)
            gripper_cmd.linear.y = 0.0;  // Stop
            arm_cmd_pub_->publish(gripper_cmd);
            
            if (elapsed < 2.0) {
                RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
                    "Stopping gripper... %.1f seconds remaining", 2.0 - elapsed);
            } else {
                // Complete - send final 0.0 signal, mark as done, and shutdown
                gripper_drop_active_ = false;
                RCLCPP_INFO(get_logger(), "Object dropped successfully! Gripper stopped.");
    ...
    void gps_callback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
    {
        ...
        {
            if (!gripper_drop_active_) {
                // Start the drop sequence
                drop_object();
            }
            // The update_gripper_drop() at the start of this callback handles the rest
            return;
        }
